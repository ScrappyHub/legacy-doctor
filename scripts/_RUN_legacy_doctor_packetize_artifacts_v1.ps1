...
$Files = @(
  ...
  (Join-Path $RepoRoot "scripts\storage\ld_packetize_artifacts_v1.ps1"),
  (Join-Path $RepoRoot "scripts\selftest\_selftest_ld_packetize_artifacts_v1.ps1")
)
...
$Selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_packetize_artifacts_v1.ps1"
...
if($joined -notmatch "SELFTEST_LD_PACKETIZE_ARTIFACTS_OK"){
  Die "SELFTEST_MISSING_OK" $Selftest
}

Write-Output "LEGACY_DOCTOR_PACKETIZE_ARTIFACTS_ALL_GREEN"