...
$files = @(
  (Join-Path $RepoRoot "scripts\storage\ld_read_probe_v1.ps1"),
  (Join-Path $RepoRoot "scripts\selftest\_selftest_ld_storage03_read_probe_v1.ps1")
)
...
$schema = Join-Path $RepoRoot "schemas\ld.device.read_probe.receipt.v1.json"
...
$selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_storage03_read_probe_v1.ps1"
...
if($outText -notmatch "SELFTEST_LD_STORAGE03_READ_PROBE_OK"){
  Die "SELFTEST_TOKEN_MISSING" ""
}

Write-Output "LEGACY_DOCTOR_STORAGE03_READ_PROBE_GREEN"
