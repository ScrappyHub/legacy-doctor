...
$files = @(
  (Join-Path $RepoRoot "scripts\storage\ld_backup_readiness_v1.ps1"),
  (Join-Path $RepoRoot "scripts\selftest\_selftest_ld_storage03_backup_readiness_v1.ps1")
)
...
$schema = Join-Path $RepoRoot "schemas\ld.device.backup_readiness.receipt.v1.json"
...
$selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_storage03_backup_readiness_v1.ps1"
...
if($outText -notmatch "SELFTEST_LD_STORAGE03_BACKUP_READINESS_OK"){
  Die "SELFTEST_TOKEN_MISSING" ""
}

Write-Output "LEGACY_DOCTOR_STORAGE03_BACKUP_READINESS_GREEN"
