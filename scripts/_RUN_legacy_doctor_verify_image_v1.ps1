...
$Files = @(
  (Join-Path $RepoRoot "scripts\storage\_lib_ld_verify_v1.ps1"),
...
$Selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_verify_image_v1.ps1"
...
if($joined -notmatch "SELFTEST_LD_VERIFY_IMAGE_OK"){
  Die "SELFTEST_MISSING_OK" $Selftest
}

Write-Output "LEGACY_DOCTOR_VERIFY_IMAGE_ALL_GREEN"