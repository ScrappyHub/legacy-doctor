...
$Files = @(
  ...
  (Join-Path $RepoRoot "scripts\storage\ld_packetize_backup_v1.ps1"),
  (Join-Path $RepoRoot "scripts\selftest\_selftest_ld_packetize_backup_v1.ps1")
)
...
$Selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_packetize_backup_v1.ps1"
...
if($joined -notmatch "SELFTEST_LD_PACKETIZE_BACKUP_OK"){
  Die "SELFTEST_MISSING_OK" $Selftest
}

Write-Output "LEGACY_DOCTOR_PACKETIZE_BACKUP_ALL_GREEN"