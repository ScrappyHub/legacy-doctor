...
$Files = @(
  ...
  (Join-Path $RepoRoot "scripts\storage\ld_pack_acquisition_v1.ps1"),
  (Join-Path $RepoRoot "scripts\selftest\_selftest_ld_pack_acquisition_v1.ps1")
)
...
$Selftest = Join-Path $RepoRoot "scripts\selftest\_selftest_ld_pack_acquisition_v1.ps1"
...
if($joined -notmatch "SELFTEST_LD_PACK_ACQUISITION_OK"){
  Die "SELFTEST_MISSING_OK" $Selftest
}

Write-Output "LEGACY_DOCTOR_PACK_ACQUISITION_ALL_GREEN"